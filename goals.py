...
with card_container(key='global'):
    ...
    with tab1:
        with card_container(key='dashboard'):
            cols = st.columns(3)
            with cols[0]:
                ui.metric_card(title="Month To Date", content=f'Ksh. {amount}', description="+20.1% \n from last month", key="card1")
            with cols[1]:
    ...
    with tab2:
        with card_container(key='form'):
            ...
            Category = st.selectbox("Category:", ["SAVINGS", "HOUSE", "FOOD", "FAMILY AND FRIENDS", "TRANSPORT", "SELF IMPROVEMENT", "ENTERTAINMENT", "WARDROBE"])        
